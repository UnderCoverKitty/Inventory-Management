...
public class PrintReceipt implements Printable {
    ...
    public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        double width = 226.77; // 80mm in points (1 mm = 2.83465 points)
        double height = 841.89; // Variable length
...
// Items - adjust the spacing to match header
        for (String[] item : items) {
            String itemLine = String.format("%-15s %3s %6s %6s",
                    truncateString(item[0], 15), // reduced from 20 to 15
                    item[2],
                    "Rs" + item[1],
                    "Rs" + item[3]);
